// State machine for the Omachron service.
//
...
// Reads the compositor's session-lock state out of `hyprctl -j monitors`.
// Hyprland reports no lock state directly, but an active ext-session-lock is
// one of the reasons a monitor cannot go solitary, so it shows up as LOCK in
// solitaryBlockedBy -- the same probe omarchy-hyprland-session-locked uses.
// Hyprland stops at the first reason on a monitor that has no workspace yet
// (WORKSPACE), before the lock is ever considered, so a LOCK missing there
// proves nothing.  A Hyprland too old to report solitaryBlockedBy reads as
// unlocked, which is the pre-probe behaviour.
// Returns true (locked), false (unlocked) or null (undetermined: the caller
// keeps its last known state).
function sessionLockFromMonitors(text) {
  var monitors
  try {
  ...
  for (var i = 0; i < monitors.length; i++) {
    var m = monitors[i]
    var blockers = m && Array.isArray(m.solitaryBlockedBy) ? m.solitaryBlockedBy : []
...
if (typeof module !== "undefined" && module && module.exports) {
  module.exports = {
    ...
    sessionLockFromMonitors: sessionLockFromMonitors
  }
}
